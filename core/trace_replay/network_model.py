...
class TraceReplayError(RuntimeError):
    """Raised when a trace-driven technical replay cannot be simulated safely."""
...
class TraceDrivenNetworkModel:
    """Technical trace replay model for normalized traces.

    This model is intentionally controller-free. It receives a loaded trace and
    a synthetic segment size, then computes the download duration implied by the
    trace samples.
    """

    def __init__(self, loaded_trace: LoadedTrace, end_policy: str = END_POLICY_FAIL, max_loops: int = 1) -> None:
    ...
    def _handle_trace_end(
        ...
        if self.end_policy == END_POLICY_FAIL:
            raise TraceReplayError("trace exhausted before delivering {0} bytes with end_policy=fail".format(requested_bytes))
        if loops_used >= self.max_loops:
            raise TraceReplayError(
                "trace exhausted before delivering {0} bytes after {1} loop(s)".format(requested_bytes, loops_used)
            )
