"""Feature construction for NeuralABR-Lite candidate scoring."""
...
class FeatureError(ValueError):
    """Raised when feature construction or validation fails."""
...
def build_context_features(state: ReplayState, ladder: ContentLadder) -> Mapping[str, object]:
    ...
    if ladder.segment_count > 0:
        chunks_remaining_norm = float(chunks_remaining) / float(ladder.segment_count)
        has_chunks_remaining = 1.0
    else:
        chunks_remaining_norm = 0.0
        has_chunks_remaining = 0.0

    return {
        "throughput_history_bps": _left_pad(state.throughput_history_bps, K_CONTEXT),
        "download_time_history_s": _left_pad(state.download_time_history_s, K_CONTEXT),
        ...
        candidates.append(
            {
                ...
                "candidate_chunk_size_bytes": float(size_bytes),
...
def flatten_context_features(context: Mapping[str, object]) -> Tuple[float, ...]:
    ...
    throughput = _numeric_sequence(context.get("throughput_history_bps"), "throughput_history_bps", K_CONTEXT)
    download_times = _numeric_sequence(context.get("download_time_history_s"), "download_time_history_s", K_CONTEXT)
...
def build_feature_schema() -> Mapping[str, object]:
    return {
        ...
        "k_context": K_CONTEXT,
        "context_fields": [
            {"name": "throughput_history_bps", "shape": [K_CONTEXT], "availability": "previous_completed_downloads_only"},
            {"name": "download_time_history_s", "shape": [K_CONTEXT], "availability": "previous_completed_downloads_only"},
            *({"name": name, "shape": [], "availability": "pre_decision_observable"} for name in CONTEXT_SCALAR_FEATURES),
        ],
        "candidate_fields": [
            {"name": name, "shape": [], "availability": "current_valid_candidate_metadata"}
            for name in CANDIDATE_FEATURES
        ],
        "context_vector_names": list(CONTEXT_VECTOR_NAMES),
        "candidate_vector_names": list(CANDIDATE_VECTOR_NAMES),
        "forbidden_model_inputs": sorted(FORBIDDEN_MODEL_INPUT_KEYS),
    }


def reject_forbidden_model_inputs(mapping: Mapping[str, object]) -> None:
    offenders = sorted(str(key) for key in mapping.keys() if str(key) in FORBIDDEN_MODEL_INPUT_KEYS)
    if offenders:
        raise FeatureError("forbidden model input key(s): {0}".format(", ".join(offenders)))
...
def audit_feature_payload(context: Mapping[str, object], candidates: Iterable[Mapping[str, object]]) -> Mapping[str, object]:
    ...
    for candidate_index, candidate in enumerate(candidates):
        try:
            ...
            errors.append("candidate {0}: {1}".format(candidate_index, exc))
