"""Small CPU-first shared MLP candidate scorer."""
...
from core.neural_abr.constants import CANDIDATE_VECTOR_NAMES, CONTEXT_VECTOR_NAMES, MODEL_CONFIG_VERSION


class ModelError(ValueError):
    """Raised when model inputs violate candidate-scoring constraints."""
...
class NeuralAbrLiteCandidateScorer(nn.Module):
    def __init__(
        ...
        input_dim = self.context_dim + self.candidate_dim
        layers = []
    ...
    def forward(self, context_features: torch.Tensor, candidate_features: torch.Tensor, action_mask: torch.Tensor) -> torch.Tensor:
        ...
        if context_features.shape[0] != candidate_features.shape[0] or action_mask.shape != candidate_features.shape[:2]:
            raise ModelError("batch/candidate dimensions do not align")
    ...
    def config(self) -> Mapping[str, object]:
        return {
            "schema_version": MODEL_CONFIG_VERSION,
...
def default_model() -> NeuralAbrLiteCandidateScorer:
    return NeuralAbrLiteCandidateScorer()
