...
from core.fase6.analisis import analizar_paquete_fase6, test_de_signos_exacto
from core.fase6.configuracion import CONFIG_FASE6_POR_DEFECTO
from core.fase6.seleccion import seleccionar_ventanas_trazas
...
from scripts.gui_fase6 import construir_comando_fase6, parsear_linea_progreso_fase6
from scripts.ejecutar_fase6 import construir_config_cliente, construir_protocolo_y_plan_fase6, ejecutar_sesion
...
class Phase6SelectionTest(unittest.TestCase):
    def test_selects_eval_windows_without_train_test_leakage(self):
        ...
        config = dict(CONFIG_FASE6_POR_DEFECTO)
    ...
    def test_filters_impossible_real_windows_but_keeps_synthetic_diagnostic(self):
        ...
        windows = seleccionar_ventanas_trazas(manifest, "rapido", dict(CONFIG_FASE6_POR_DEFECTO))
...
class Phase6AnalysisTest(unittest.TestCase):
    def test_analyzes_package_and_writes_validation_artifacts(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            package = analizar_paquete_fase6(root, generate_plots=False)
    ...
    def test_plot_manifest_is_generated_with_minimal_dataset(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            analizar_paquete_fase6(root, generate_plots=True)
    ...
    def test_package_verifier_passes_correct_package_and_fails_failed_package(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            analizar_paquete_fase6(root, generate_plots=False)

            ok = verificar_paquete_fase6(root, require_plots=False)
            ...
            analizar_paquete_fase6(failed_root, generate_plots=False)

            bad = verificar_paquete_fase6(failed_root, require_plots=False)
...
class Phase6RunnerAndGuiTest(unittest.TestCase):
    def test_builds_protocol_and_client_config(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            protocol, sessions = construir_protocolo_y_plan_fase6(config, "rapido", package_root)
    ...
    def test_diagnostico_preset_builds_short_full_pipeline_plan(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            protocol, sessions = construir_protocolo_y_plan_fase6(config, "diagnostico", tmp_path / "package")
            client_config = construir_config_cliente(config, sessions[0])
    ...
    def test_gui_progress_line_parser(self):
        parsed = parsear_linea_progreso_fase6(
            "FASE6_PROGRESO processed=7 total=70 percent=10.0 executed=7 failed=1 skipped=0 elapsed_s=120.5 last_session_s=20.0 avg_session_s=17.2 eta_s=1083.6 session=s00007\n"
        )
    ...
    def test_runner_tolerates_non_utf8_child_output(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch("scripts.ejecutar_fase6.subprocess.run", return_value=completed):
