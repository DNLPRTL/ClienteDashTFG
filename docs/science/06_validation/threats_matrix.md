...
| Phase 4 identity leakage | Phase 4 candidate data had checksum/content identity leakage risk and cannot be strong generalization evidence | Phase 6 must block overlap by `trace_id`, `leakage_group`, `checksum_sha256` and `canonical_content_fingerprint` | Eligibility audit before any evaluation |
| Missing canonical content fingerprint parsing | Closed in Phase 6B | `scripts/audit_phase6_trace_eligibility.py` now parses, reports and blocks `canonical_content_fingerprint` separately from `checksum_sha256` | Future manifests must keep both identity fields auditable |
