...
## Controller-Added Diagnostic Columns

`NeuralAbrLiteController.augment_feedback()` may add the following feedback keys when `neural_abr_lite` is selected. `player.Player._copy_post_decision_feedback_telemetry()` may then copy values from `NeuralAbrLiteController.get_last_decision_telemetry()` into existing `segment_telemetry.csv` columns after `calcControlAction()`.

These columns are diagnostic-only structural telemetry. They are not written to `evaluation_segments.csv`, are not final QoE or ranking inputs, and do not define benchmark evidence, improvement claims, retraining labels, or Phase 6 comparative validation.

| Column | Artifact | Producer module/function | Unit | Timing/source event | Semantic status | Direct benchmark use now? | Notes/risks |
|---|---|---|---|---|---|---|---|
| `feedback_neural_enabled` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.augment_feedback` / `get_last_decision_telemetry` | boolean int | Header/row default before decision; updated after decision when available. | debug/test-only; runtime-only | No | Shows whether the guarded neural path is active for the controller instance. |
| `feedback_neural_bundle_configured` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.augment_feedback` / `get_last_decision_telemetry` | boolean int | Header/row default before decision; updated after decision when available. | debug/test-only; runtime-only | No | Local bundle path configured flag only; no artifact path is written by this field. |
| `feedback_neural_bundle_loaded` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.augment_feedback` / `get_last_decision_telemetry` | boolean int | Header/row default before decision; updated after decision when available. | debug/test-only; runtime-only | No | Bundle load state; not a quality or performance score. |
| `feedback_neural_bundle_schema_ok` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.augment_feedback` / `get_last_decision_telemetry` | boolean int | Header/row default before decision; updated after decision when available. | debug/test-only; runtime-only | No | Schema validation status for local bundle metadata. |
| `feedback_neural_bundle_hash_ok` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.augment_feedback` / `get_last_decision_telemetry` | boolean int | Header/row default before decision; updated after decision when available. | debug/test-only; runtime-only | No | Hash validation status for local bundle payload files. |
| `feedback_neural_feature_schema_ok` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.augment_feedback` / `get_last_decision_telemetry` | boolean int | Header/row default before decision; updated after decision when available. | debug/test-only; runtime-only | No | Feature schema compatibility status. |
| `feedback_neural_feature_vector_ok` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.get_last_decision_telemetry` | boolean int | After controller decision. | debug/test-only; runtime-only | No | Runtime feature construction status for one decision. |
| `feedback_neural_missing_features` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.get_last_decision_telemetry` | CSV-safe text | After controller decision. | debug/test-only; runtime-only | No | Missing runtime feature names, if any. |
| `feedback_neural_action_mask_valid_count` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.get_last_decision_telemetry` | count | After controller decision. | debug/test-only; runtime-only | No | Number of currently valid MPD ladder actions. |
| `feedback_neural_raw_action` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.get_last_decision_telemetry` | representation index | After controller decision. | debug/test-only; runtime-only | No | Raw scorer-selected representation index before safety guard. |
| `feedback_neural_raw_rate_Bps` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.get_last_decision_telemetry` | bytes per second | After controller decision. | debug/test-only; runtime-only | No | Raw action mapped to an existing MPD ladder rate. |
| `feedback_neural_safe_action` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.get_last_decision_telemetry` | representation index | After controller decision. | debug/test-only; runtime-only | No | Executed or fallback representation index. |
| `feedback_neural_safe_rate_Bps` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.get_last_decision_telemetry` | bytes per second | After controller decision. | debug/test-only; runtime-only | No | Executed or fallback ladder rate. |
| `feedback_neural_safety_intervened` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.get_last_decision_telemetry` | boolean int | After controller decision. | debug/test-only; runtime-only | No | Safety guard intervention flag; not a success metric. |
| `feedback_neural_fallback_used` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.get_last_decision_telemetry` | boolean int | After controller decision. | debug/test-only; runtime-only | No | Fallback path flag; diagnostic only. |
| `feedback_neural_fallback_reason` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.get_last_decision_telemetry` | reason label | After controller decision. | debug/test-only; runtime-only | No | Stable fail-closed reason label. Unknown exception-like text is normalized to an enum-style fallback label before CSV output. |
| `feedback_neural_inference_ms` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.get_last_decision_telemetry` | milliseconds | After controller decision. | debug/test-only; runtime-only | No | Local CPU inference timing; not benchmark-grade latency evidence. |
| `feedback_neural_nan_inf_detected` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.get_last_decision_telemetry` | boolean int | After controller decision. | debug/test-only; runtime-only | No | Non-finite score detection flag. |
| `feedback_neural_invalid_action_detected` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.get_last_decision_telemetry` | boolean int | After controller decision. | debug/test-only; runtime-only | No | Masked or out-of-ladder selected action detection flag. |
| `feedback_neural_diagnostic_only` | `segment_telemetry.csv` | `core.controller.neural_abr_lite.NeuralAbrLiteController.augment_feedback` / `get_last_decision_telemetry` | boolean int | Header/row default before decision; updated after decision when available. | debug/test-only; runtime-only | No | Explicit boundary marker: diagnostic-only, not comparison evidence. |
